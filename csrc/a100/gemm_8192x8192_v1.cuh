...
__global__ void mmaSlowKernel(const half *__restrict__ A,
...
#pragma unroll 1
    for (size_t tile_k = 0; tile_k < K_tiles; tile_k += CHUNK_K) {
        ...
        // ==================================================================
        {
            size_t A_smem_row = warp_id * (size_t)WARP_SIZE + lane_id;
            const half *A_gmem_row_ptr =
                A_block_ptr + A_smem_row * K + tile_k * (size_t)MMA_K;

#pragma unroll
            for (size_t h = 0; h < (size_t)AB_SMEM_STRIDE; ++h) {
                smem[A_smem_row][h] = A_gmem_row_ptr[h];
            }
        }
        ...
        // ==================================================================
        {
            size_t B_smem_row_local = warp_id * 16 + (lane_id % 16);
            size_t col_base         = (lane_id / 16) * 16;
            const half *B_gmem_row_ptr =
                B_block_ptr + B_smem_row_local * K + tile_k * (size_t)MMA_K;

#pragma unroll
            for (size_t h = 0; h < 16; ++h) {
                smem[B_smem_idx_off + B_smem_row_local][col_base + h] =
                    B_gmem_row_ptr[col_base + h];
            }
        }
...
static size_t initMmaSlow() {
    ...
    CUDA_CHECK(cudaFuncSetAttribute(
        mmaSlowKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, (int)smem_max_size));

    return smem_max_size;
}

static void launchKernel(half *dA, half *dB, half *dC, int M, int N, int K) {
    static size_t smem_max = initMmaSlow();
    ...
    mmaSlowKernel<<<grid, block, smem_max>>>(dA, dB, dC, M, N, K);
}

} // namespace gemm_8192x8192_v1

using gemm_8192x8192_v1::launchKernel;
