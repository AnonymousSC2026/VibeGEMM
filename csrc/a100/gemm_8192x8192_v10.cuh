...
// Fixed: use f32 accumulators, 4 outputs per thread, in-place accumulate with +f
...
// Pipeline depth
#define K_STAGE 3
// Rows per stage in the AB smem 2D view (stride = AB_SMEM_STRIDE halfs)
...
#pragma unroll
    ...
    const size_t NI = div_ceil(K_tiles, (size_t)CHUNK_K);   // # of compute iterations
...
#define LOAD_AB_TO_REG(REG_IDX, SMEM_SLOT, K_STEP)                                         \
    ...
    // Fixed: HMMA_ROW must also pass RC[2], RC[3], and remove duplicated tail RC args
...
// C store fixed for float RC[...][...][4]:
// convert two float -> half2, preserve original swizzled smem layout
#pragma unroll
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        ...
        *((half2 *)(lane_ptr0)) = __floats2half2_rn(RC[i][j][0], RC[i][j][1]);
        *((half2 *)(lane_ptr1)) = __floats2half2_rn(RC[i][j][2], RC[i][j][3]);
    }
}
    __syncthreads();
