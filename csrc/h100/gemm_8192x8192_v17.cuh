...
matmulKernel(
    ...
    Schedule<BM * Shape::ClusterM, BN * Shape::ClusterN, 16 / Shape::ClusterM, 8 / Shape::ClusterN> schedule(M, N, clusterId, clusterCount);
            ...
            while (schedule.next(numBlockM, numBlockN)) {
                ...
                for (int blockKIter = 0; blockKIter < numBlocksK; ++blockKIter, ++qidx) {
                    ...
                    if constexpr (Shape::ClusterN > 1) {
                        ...
                        if (rankN == 0) {
                            loadAsyncMulticast(&sA[qidx * BK * BM], &tensorMapA, &full[qidx], blockKIter * BK, numBlockM * BM, mask);
                        }
                    } else {
                    ...
                    if constexpr (Shape::ClusterM > 1) {
                        if (rankM == 0) {
                            loadAsyncMulticast(&sB[qidx * BK * BN], &tensorMapB, &full[qidx], blockKIter * BK, numBlockN * BN, colMask << rankN);
                        }
                    } else {
...
void launchKernel(half* A, half* B, half* C, int M, int N, int K) {
    ...
    kptr<<<NUM_SM, NUM_THREADS, sharedBytes>>>(M, N, K, globalTmaMapA, globalTmaMapB, globalTmaMapC);
}

}

using gemm_8192x8192_v17::launchKernel;