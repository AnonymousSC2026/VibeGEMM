...
__device__ __forceinline__ void wgmmaWait0() {
...
__device__ __forceinline__ void mapThreadToEpilogue(int tid, int wgIdx, int wgMTiles, int mIt,
                                                    int nIt, int w, int &row, int &col) {
    const int wgTid = tid & 127;
    const int lane = wgTid & 31;
    const int wgWarp = wgTid >> 5;
    const int rowInTile = wgWarp * 16 + (lane >> 2);
    const int yo = (wgIdx * wgMTiles + mIt) * 64;
    const int colInTile = 16 * w + 2 * (lane & 3);

    row = yo + rowInTile;
    col = nIt * 64 + colInTile;
}
...
#pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    float acc[Shape::NumWgmmaM][Shape::NumWgmmaN][32];
...
#pragma unroll
            for (int mi = 0; mi < Shape::NumWgmmaM; ++mi) {
                half *ptrA = &smem.A[0];
                wgmmaM64N64K16<0>(acc[mi][0], &ptrA[0], &smem.B[0]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[Shape::WgmmaK], &smem.B[Shape::WgmmaK]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[2 * Shape::WgmmaK], &smem.B[2 * Shape::WgmmaK]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[3 * Shape::WgmmaK], &smem.B[3 * Shape::WgmmaK]);
            }
        } else {
#pragma unroll
            for (int mi = 0; mi < Shape::NumWgmmaM; ++mi) {
                half *ptrA = &smem.A[0];
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[0], &smem.B[0]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[Shape::WgmmaK], &smem.B[Shape::WgmmaK]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[2 * Shape::WgmmaK], &smem.B[2 * Shape::WgmmaK]);
                wgmmaM64N64K16<1>(acc[mi][0], &ptrA[3 * Shape::WgmmaK], &smem.B[3 * Shape::WgmmaK]);
            }
    ...
    for (int mi = 0; mi < Shape::NumWgmmaM; ++mi) {
        for (int ni = 0; ni < Shape::NumWgmmaN; ++ni) {
            for (int w = 0; w < Shape::WgmmaN / 16; ++w) {
                int row, col;
                mapThreadToEpilogue(threadIdx.x, wgIdx, Shape::NumWgmmaM, mi, ni, w, row, col);
...
static int prevM = 0, prevN = 0, prevK = 0;
...
void launchKernel(half *A, half *B, half *C, int M, int N, int K) {
    ...
    gemm<Shape><<<grid, block>>>(M, N, K, C, globalTmaMapA, globalTmaMapB);
}

} // namespace gemm_8192x8192_v2

using gemm_8192x8192_v2::launchKernel;