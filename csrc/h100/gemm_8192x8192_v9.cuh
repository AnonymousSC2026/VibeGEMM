...
template <int BlockMajorSize, int BlockMinorSize>
__host__ CUtensorMap* createTensorMapDevice(half* src, int blocksHeight, int blocksWidth) {
    CUtensorMap *tmaMapDevice;
    cudaMalloc(&tmaMapDevice, sizeof(CUtensorMap));
    CUtensorMap tmaMapHost;
    createTensorMap<BlockMajorSize, BlockMinorSize>(&tmaMapHost, src, blocksHeight, blocksWidth);
...
matmulKernel(int M, int N, int K, half* __restrict__ C, const CUtensorMap* __restrict__ tensorMapA, const CUtensorMap* __restrict__ tensorMapB) {
        ...
        if (wgIdx == 0) {
            if (tid128 == 0) {
                for (int kIter = 0; kIter < numBlocksK; ++kIter) {
                    ...
                    cde::cp_async_bulk_tensor_2d_global_to_shared(sA, tensorMapA, kIter * BK, numBlockM * BM, full[q]);
                    cde::cp_async_bulk_tensor_2d_global_to_shared(sB, tensorMapB, kIter * BK, numBlockN * BN, full[q]);
...
CUtensorMap* globalTmaMapA = nullptr;
CUtensorMap* globalTmaMapB = nullptr;
int prevM = 0;
int prevN = 0;
int prevK = 0;
...
void launchKernel(half* A, half* B, half* C, int M, int N, int K) {
    ...
    if (!globalTmaMapA || M != prevM || N != prevN || K != prevK) {
        ...
        globalTmaMapA = createTensorMapDevice<BM, BK>(A, M / BM, K / BK);
        globalTmaMapB = createTensorMapDevice<BN, BK>(B, N / BN, K / BK);
        prevM = M;
        prevN = N;
        prevK = K;
    }
    ...
    kptr<<<numSm, NUM_THREADS, sharedBytes>>>(M, N, K, C, globalTmaMapA, globalTmaMapB);
}

}

using gemm_8192x8192_v9::launchKernel;