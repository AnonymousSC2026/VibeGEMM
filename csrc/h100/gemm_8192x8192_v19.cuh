...
__device__ __forceinline__ void loadAsyncMulticast(half *dst, const CUtensorMap *srcTma,
...
__device__ __forceinline__ void loadAsync(half *dst, const CUtensorMap *srcTma, uint64_t *bar,
...
__device__ __forceinline__ void storeAsync(const CUtensorMap *dstTmaMap, half *src,
                                           int globalColIdx, int globalRowIdx) {
...
__host__ void createTensorMap(CUtensorMap *tmaMap, half *gmemPtr, int globalHeight,
                              int globalWidth) {
...
#pragma unroll
            ...
            while (schedIt < 128) {
                int v = smem.space[schedIt++];
                if (v < 0)
                    break;

                numBlockM = (v >> 16) & 0xFFFF;
                numBlockN = v & 0xFFFF;
...
#pragma unroll
            ...
            auto storeC = [&](int i, int j, float val) { blockSc[j * B_WG_M + i] = (half)val; };
...
static int *generateAdvancedSwizzleHostData(int mTiles, int nTiles, int numClusters) {
    const int depth = 128;
    ...
    static_assert((SWIZZLE & (SWIZZLE - 1)) == 0, "SWIZZLE must be power of 2");

    int *h = (int *)malloc((size_t)numClusters * depth * sizeof(int));
    for (int i = 0; i < numClusters * depth; ++i)
        h[i] = -1;

    const int macroM = (mTiles + BM - 1) / BM;
    const int macroN = (nTiles + BN - 1) / BN;

    int taskId = 0;

    for (int mm = 0; mm < macroM; ++mm) {
        const bool reverseN = (mm & 1);

        for (int nn0 = 0; nn0 < macroN; ++nn0) {
            int nn = reverseN ? (macroN - 1 - nn0) : nn0;

            const int mBase = mm * BM;
            const int nBase = nn * BN;

            for (int mi = 0; mi < BM; ++mi) {
                const int blockM = mBase + mi;
                if (blockM >= mTiles)
                    continue;

                for (int nb = 0; nb < BN; nb += SWIZZLE) {
                    for (int s = 0; s < SWIZZLE; ++s) {
                        int local = (s ^ (blockM & (SWIZZLE - 1)));
                        int ni = nb + local;
                        if (ni >= BN)
                            continue;

                        const int blockN = nBase + ni;
                        if (blockN >= nTiles)
                            continue;
                        ...
                        if (pos < depth) {
                            h[cluster * depth + pos] = (blockM << 16) | (blockN & 0xFFFF);
                        }
...
void launchKernel(half *A, half *B, half *C, int M, int N, int K) {
    ...
    const bool needRegen = (globalDspace == nullptr) || (spaceMTiles != mTiles) ||
                           (spaceNTiles != nTiles) || (spaceNumClusters != numClusters);
    ...
    if (needRegen) {
        ...
        int *hSpace = generateAdvancedSwizzleHostData(mTiles, nTiles, numClusters);
        cudaMalloc(&globalDspace, (size_t)numClusters * 128 * sizeof(int));
        cudaMemcpy(globalDspace, hSpace, (size_t)numClusters * 128 * sizeof(int),
    ...
    kptr<<<NUM_SM, NUM_THREADS, sharedBytes>>>(M, N, K, globalTmaMapC, globalTmaMapA, globalTmaMapB,
                                               globalDspace);
}

} // namespace gemm_8192x8192_v19

using gemm_8192x8192_v19::launchKernel;