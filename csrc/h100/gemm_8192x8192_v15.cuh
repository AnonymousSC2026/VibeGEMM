...
__device__ __forceinline__ void loadAsyncMulticast(half *dst, void const *const srcTma,
                                                   uint64_t *bar, int gCol, int gRow,
...
__device__ __forceinline__ void tmaLoad5D(half *smemPtr, const CUtensorMap *map, int coordK64,
...
template <int BM, int BN, int TM = 16, int TN = 8> struct Schedule {
    ...
    __device__ __forceinline__ Schedule(int M, int N, int _block) : block(_block), it(0) {
        totalBlocksM = M / BM;
        totalBlocksN = N / BN;
        numSm = (int)gridDim.x;
    }
...
__global__ void __launch_bounds__(Shape::NumThreads) __cluster_dims__(Shape::ClusterSize, 1, 1)
    matmulKernel(int M, int N, int K, half *__restrict__ C,
...
#pragma unroll
        for (int i = 0; i < QSIZE; ++i) {
            mbarrierInit(&full[i], 1);
            mbarrierInit(&empty[i], numConsumers);
        }
    }
    ...
    Schedule<BM, BN, 16, 8> schedule(M, N, blockIdx.x);
            ...
            while (schedule.next(blockM, blockN)) {
                for (int kIter = 0; kIter < numBlocksK; ++kIter, ++qidx) {
                    ...
                    if constexpr (Shape::ClusterN > 1) {
                        ...
                        if (rankN == 0) {
                            loadAsyncMulticast(&smem.A[qidx * BM * BK], &tensorMapA, &full[qidx],
                                               kIter, blockM * BM, mask);
                        }
                    } else {
                        tmaLoad5D(&smem.A[qidx * BM * BK], &tensorMapA, kIter, blockM * BM,
                                  &full[qidx]);
                    }

                    tmaLoad5D(&smem.B[qidx * BK * BN], &tensorMapB, kIter, blockN * BN,
                              &full[qidx]);
                }
            }
        }
...
#pragma unroll
                for (int mIt = 0; mIt < B_WG_M / WGMMA_M; ++mIt) {
                    half *wgmmaSa =
                        sABase + 64 * (mIt + consumerIdx * (B_WG_M / WGMMA_M)) * WGMMA_M;
                    half *wgmmaSb = sBBase;
...
#pragma unroll
            ...
            auto storeC = [&](int i, int j, float val) { cBlock[j * M + i] = (half)val; };
...
using gemm_8192x8192_v15::launchKernel;