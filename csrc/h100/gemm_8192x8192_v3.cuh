...
__device__ __forceinline__ void mapThreadToEpilogue(int tid, int wgIdx, int wgMTiles, int mIt,
    ...
    const int rOffset = (wgIdx * wgMTiles + mIt) * 64;
    const int colInTile = 16 * w + 2 * (lane & 3);

    row = rOffset + rowInTile;
...
#pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    float acc[Shape::NumWgmmaM][Shape::WgmmaN / 16][8];
...
#pragma unroll
        for (int mi = 0; mi < Shape::NumWgmmaM; ++mi) {
            half *ptrAM = &smem.A[mi * Shape::WgmmaM * Shape::TileK];

#pragma unroll
            for (int ki = 0; ki < Shape::NumItersK; ++ki) {
                half *ptrA = &ptrAM[ki * Shape::WgmmaK];
                half *ptrB = &smem.B[ki * Shape::WgmmaK];

                if (kStep == 0 && ki == 0) {
                    wgmmaM64N64K16<0>(acc[mi], ptrA, ptrB);
                } else {
                    wgmmaM64N64K16<1>(acc[mi], ptrA, ptrB);
                }
            }
        }
