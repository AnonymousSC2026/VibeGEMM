...
__device__ __forceinline__ void mapThreadToEpilogue(
    ...
    row = mIt * wgmmaM + rowInTile;
    col = nIt * wgmmaN + colInTile;
}
...
void createTensorMap(CUtensorMap *tmaMap, half *gmemPtr, int blocksHeight, int blocksWidth) {
...
template <int BlockMajorSize, int BlockMinorSize>
__host__ __forceinline__ CUtensorMap *allocateAndCreateTensorMap(half *src, int blocksHeight, int blocksWidth) {
...
#pragma nv_diag_suppress static_var_with_dynamic_init
    ...
    if (wgIdx == 0) {
        ...
        float d[Shape::TileM / Shape::WgmmaM][Shape::WgmmaN / 16][8] = {0.0f};
...
#pragma unroll
            for (int mBlk = 0; mBlk < Shape::TileM / Shape::WgmmaM; ++mBlk) {
#pragma unroll
                for (int kStep = 0; kStep < Shape::TileK / 16; ++kStep) {
                    wgmmaM64N128K16<1, 1, 1, 0, 0>(
                        reinterpret_cast<float (*)[8]>(d[mBlk]),
                        sA + mBlk * Shape::WgmmaM * Shape::TileK + kStep * 16,
                        sB + kStep * 16
                    );
                }
            }
        ...
        for (int mIt = 0; mIt < Shape::TileM / Shape::WgmmaM; ++mIt) {
...
void launchKernel(half *A, half *B, half *C, int M, int N, int K) {
    ...
    if (!globalTmaMapA || M != prevM || N != prevN || K != prevK) {
        ...
        globalTmaMapA = allocateAndCreateTensorMap<BM, BK>(A, M / BM, K / BK);
        globalTmaMapB = allocateAndCreateTensorMap<BN, BK>(B, N / BN, K / BK);
